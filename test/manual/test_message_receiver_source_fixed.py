...
def test_message_receiver_source():
    ...
    try:
        ...
        try:
            ...
            print(f"   [ERROR] 钉钉模式应该抛出异常但没有")
            return False
        except ValueError as e:
            if "dingtalk-service" in str(e):
                print(f"   [OK] 钉钉模式正确抛出友好异常")
                print(f"   错误信息: {e}")
            else:
                print(f"   [ERROR] 钉钉模式抛出错误异常: {e}")
                return False
        except Exception as e:
            print(f"   [ERROR] 钉钉模式抛出错误异常类型: {e}")
            return False
